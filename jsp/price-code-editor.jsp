...
<%
	...
	String htmlTitle = "Price Code Editor";
%>
...
function HideLayer()
{
    ...
    document.getElementById("SellCodeSearch").value ="";
	document.getElementById("RetailCodeSearch").value="";
	document.getElementById("DescriptionSearch").value="";
	document.getElementsByName("IsDigitalSearch")[2].checked = true;  // default to Both
...
//global variables needed in include price-code-editor-javascript.js
  var sort = "<%= MilestoneConstants.CMD_PRICE_CODE_SORT%>";
  var editor = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_PRICE_CODE_EDITOR)%>";
  var notepad = "<%= MilestoneConstants.NOTEPAD_PRICECODE%>";
  var cmd = "<%= form.getRenderableValue("cmd")%>";
  var deleteCommand = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_PRICE_CODE_EDIT_DELETE) %>";
  var save = "<%= MilestoneConstants.CMD_PRICE_CODE_EDIT_SAVE%>";
  var saveNew = "<%= MilestoneConstants.CMD_PRICE_CODE_EDIT_SAVE_NEW%>";
  var editNew = "<%= MilestoneConstants.CMD_PRICE_CODE_EDIT_NEW%>";
  var search = "<%= MilestoneConstants.CMD_PRICE_CODE_SEARCH%>";
...
function submitResize()
{
  ...
  parent.top.bottomFrame.location = "home?cmd=notepad-toggle&notepadType=<%=MilestoneConstants.NOTEPAD_PRICECODE_VISIBLE%>&lastCommand=<%=form.getRenderableValue("cmd")%>";
...
<script type="text/javascript" language="JavaScript" src="<%= inf.getHtmlDirectory() %>price-code-editor-javascript.js"></script>
...
<% if (form.getRenderableValue("cmd").equals("release-week-editor"))
	{
%>
	<body topmargin="0" leftmargin="0" onLoad="processLoad('RetailCode');">
<%}
	else
	{
%>
	<body topmargin="0" leftmargin="0" onLoad="processLoad('SellCode');">
<%} %>
...
<tr valign="middle" align="left">
	...
	<td width="170"><span class="title">Price Code</span></td>
  <td width="85"></td>
	<td width="85"></td>
	<td width="85">
	<div align="center">
...
<tr>
 ...
 %>
  	<% /* ===============start users navigation panel===========================
		...
		%>
       	 <%@ include file="include-price-code-security-notepad.shtml" %>
  ...
  %>
	<td <%if(((FormCheckBox)form.getElement("IsDigital")).isChecked()) {%>bgcolor="#F3DBDF"<%} else {%>bgcolor="lavender"<%}%> valign="top" colspan="6" width="100%">
		<table width="100%" height="100%" cellpadding="5" border="0">
		<tr>
			<td valign="top" width="100%">
				<table width="100%">
				<tr>
					<td class="label">Sell Code</td>
					<td colspan="2">
				  <% if (form.getRenderableValue("cmd").equals("price-code-editor"))
						 {
						%>
	 						<%= form.getRenderableValue("SellCode")%>
						<%}
							else
							{%>
							<%= form.getElement("SellCode") %>
						<%} %>
					</td>
				</tr>
				<tr>
					<td class="label">Retail Code</td>
					<td colspan="2"><%= form.getElement("RetailCode") %></td>
				</tr>
				<tr>
					<td class="label">Units</td>
					<td><%= form.getElement("Units") %></td>
				</tr>
				<tr>
					<td class="label">Price Range</td>
					<td><%= form.getElement("PricePoint") %></td>
				</tr>
				<tr>
				...
				<tr>
					<td class="label">R1 Cost</td>
					<td><%= form.getElement("UnitCost") %></td>
				</tr>
				<tr>
					<td class="label">Retail Price</td>
					<td><%= form.getElement("TotalCost") %></td>
				</tr>
				<tr>
					<td class="label"><b>Digital Price Code</b></td>
					<td><%= form.getElement("IsDigital") %></td>
				</tr>
...
<table width="100%" border="1" cellspacing="0"><tr><td>
	<table width="100%">
	<tr><th colspan="2">Price Code Search</th></tr>
	<tr>
		<td class="label">Sell Code</td>
		<td><%= form.getElement("SellCodeSearch") %></td>
	</tr>
	<tr>
		<td class="label">Retail Code</td>
		<td><%= form.getElement("RetailCodeSearch") %></td>
	</tr>
	<tr>
		<td class="label">Description</td>
	...
	<tr>
		<td class="label">Price Code</td>
		<td><%= form.getElement("IsDigitalSearch") %></td>
	</tr>
