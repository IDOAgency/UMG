...
<%
  ...
  boolean copyDigitalVisible = false; // release upgrade 8.1 hide buttons
  ...
  copyVisible = ((String)context.getDelivery("copyVisible")).equals("true"); // release upgrade 8.1 hide buttons
  copyDigitalVisible = ((String)context.getDelivery("copyVisible")).equals("true");
  ...
  //System.out.println("IsUseWCFServices...." + MilestoneConstants.IsUseWCFServices);
  
  // release upgrade 8.1 hide buttons
   if(MilestoneConstants.IsGDRSactive)
   {
    copyDigitalVisible = false;
   }
  // release upgrade 8.1 hide buttons
...
//global variables needed in include selection-editor-javascript.js
  var IsGDRSactive = <%=MilestoneConstants.IsGDRSactive%>;   // MC GDRS 11/06/2008
  ...
  //preset previousReleaseType with the release type value
  var previousReleaseType;//jo ITS 44669 

// JR - ITS 832
function ReleasingFamilyChange(obj){

  // show/hide the Generate Local product number button
  if(document.all.glp)
  {
    if (obj.options[obj.selectedIndex].text == "<%=MilestoneConstants.CANADA%>")
    {
      document.all.glpAnchor.href = "#";  // disable the anchor
      mtbLPNDown.isOff = false;
      mtbLPNDown.click();
      document.all.glp.style.visibility = "hidden";
    } else {
      document.all.glpAnchor.href = "JavaScript:submitGenerateLPN('LPNDown')"; // enable the anchor
      mtbLPNDown.isOff = true;
      mtbLPNDown.isHighlighted = true;
      mtbLPNDown.click();
      mtbLPNDown.out()
      document.all.glp.style.visibility = "visible";
    }
  }
  return;
}
...
<%=form.getElement("generateSelection")%>
 ...
 <%=((FormHidden)form.getElement("GDRSProductStatus")).render()%>
 
 <%=((FormHidden)form.getElement("ForceNoDigitalRelease")).render()%>

<!-- msc 12/13/01 -->
<input type="hidden" name="alphaGroupChr" value="">

<!-- JP Digital AFE -->
<input type="hidden" name="isDigital" value="0">
...
<table width="768" cellpadding="0" cellspacing=0 border="0">
  <tr valign="center" align="left">
    ...
    <td width="85">
        ...
        <script type="text/javascript" language="javascript">
          var mtbCopy = new ToggleButton(document, '<%= inf.getImageDirectory() %>', 'Copy', "JavaScript:submitCopy()", 66, 14);
        </script>
        <%}%>
      </div>
		</td>
    ...
    <td width="85">
			<div align="center" id="copyDigitalDiv">
        <%if(copyDigitalVisible)
        {%>
        <a href="JavaScript:submitCopyDigital()"
          onMouseOver="Javascript:mtbCopyDigital.over();return true;"
          onMouseOut="Javascript:mtbCopyDigital.out()"
          onClick="Javascript:mtbCopyDigital.click(); return true;">
          <img name="Copy_Digital" id="Copy_Digital" src="<%= inf.getImageDirectory() %>Copy_Digital_On.gif" width="104" height="14" border="0" hspace="0" vspace="0" align="absmiddle">
        </a>
        <script type="text/javascript" language="javascript">
          var mtbCopyDigital = new ToggleButton(document, '<%= inf.getImageDirectory() %>', 'Copy_Digital', "JavaScript:submitCopyDigital()", 104, 14);
        </script>
        <%}%>
      </div>
		</td>
 ...
 <tr><td colspan=8 height=10>&nbsp;</td></tr>
  ...
  <% //MAIN TABLE%>
  <table width="100%" bgcolor="lavender" border="0" cellspacing=0 cellpadding=0 style="padding-bottom:1px;padding-top:1px;padding-right:2px;padding-left:2px">
  <!-- JR New Selection Screen Layout PNR driven 12/03/02 -->
	...
	<tr>
		...
		<td width=10>&nbsp</td>
		<td><!--<b>Version</b>&nbsp;--></td>
		<td><%//= form.getElement("sideATitle") %></td>
	</tr>
	<tr>
		...
		<td>&nbsp</td>
		<td><b>B Side</b>&nbsp;</td>
		<td><%= form.getElement("sideBTitle") %></td>
	</tr>
	<tr>
		<td><b><font color=red>*</font> Product Title</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	...
	<!-- SECTION II -->
	<tr>
		<td nowrap><b><b><font color=red>*</font> Street/Ship Date</b></td>
		<td nowrap colspan=4>
			<%= form.getElement("streetDate")%>
      <% if (form.getRenderableValue("dayType") != null)
       ...
       <%}%>
			...
			<%}%>
			&nbsp;&nbsp;<b>Rls Wk/Cycle</b>&nbsp;&nbsp;<%=releaseWeek%>
			&nbsp;&nbsp;<b><font color=red>*</font>&nbsp;Status</b>&nbsp;&nbsp;<%= form.getElement("status") %>
		</td>
	</tr>
	<tr>
		<td><%if(!MilestoneConstants.IsGDRSactive) {%>
				<b>Digital Release Date</b>
			<%}%>
		</td>
		    <td nowrap>
                    <!--jo 12/22/2005 ITS 44669
                    The digital date is no longer an editable text box. The digital date is updated
                    when the physical date is changed for Commercial product, or, when the "No Digital Release"
                    checkbox is selected/deselected.
                    -->
                    <%= ((FormHidden)form.getElement("digitalDate")).render()%>
                    <% //pad out the digital date string so it takes up 8 chars
                    ...
                    if (form.getRenderableValue("digitalDateDisplay").length()>0){
                          digitalReleaseDate = form.getRenderableValue("digitalDateDisplay");
                    }
                    %>
                    <span <%if(MilestoneConstants.IsGDRSactive) {%>style="display:none"<%}%> id="digitalReleaseDate"><%= digitalReleaseDate %></span>

                    <%= form.getElement("noDigitalRelease") %><b>No Digital Release</b>&nbsp;&nbsp;
		    <td nowrap colspan=2>
			<%= form.getElement("holdIndicator") %><b>Hold</b>&nbsp;&nbsp;
			<%
				// this checks to see what image to display
				// if there are existing hold reasons: display afile.gif
				// if no hold reasons exist: display file.gif
				// keep in mind this is a read-only field with only a cancel button
				String holdReason = "";
				holdReason = form.getRenderableValue("holdReason");
				if (holdReason != null && !holdReason.equals(""))
				{
			%>
			<img name="holdReasonImage" title="Hold Comments" src="<%= inf.getImageDirectory() %>afile.gif" border="0" onClick="toggle( 'holdReasonLayer', 'holdReason' )">
			<%}
				else
				{
			%>
			<img name="holdReasonImage" title="Hold Comments" src="<%= inf.getImageDirectory() %>file.gif" border="0" onClick="toggle( 'holdReasonLayer', 'holdReason' )">
			<%}%>
			&nbsp;
		</td>
               <td>
			<%= form.getElement("intlFlag") %><b>Int'l Only</b>
                        &nbsp;
                        <%= form.getElement("pdIndicator") %><b>P&D / Dist.</b>
               </td>
	</tr>
	<tr>
		<td><b>Int'l Date</b></td>
		<td nowrap><%= form.getElement("internationalDate") %>&nbsp;&nbsp;</td>
		<td colspan=2>&nbsp;
		<%
    String linkStyle = "";
	...
	<td colspan=3 nowrap>
         ...
         <!-- mc its 1069 Distribution Company -->
         <!--  jo its 1069 - added code for case of New Selection from Project Search -->
        <b>&nbsp;&nbsp;&nbsp;&nbsp;Dist Company&nbsp;&nbsp;</b>
        <%
        ...
        if (currentSelection != null && currentSelection.getLabel() == null){
               distCoName = MilestoneHelper_2.getLabelDistCo(Integer.parseInt(form.getElement("distCoLabelID").getDisplayName()));
        } else if (currentSelection != null && currentSelection.getLabel() != null){
               distCoName = MilestoneHelper_2.getLabelDistCo(currentSelection.getLabel().getStructureID());
        } else {
               distCoName = "";
        }
        %>
        <span id="distCoDiv"><%=distCoName%></span></td>
        </tr>

	<tr>
	<td><b><font color=red>*</font> Environment</td>
	<td align="left"><%= ((FormHidden)form.getElement("environment")).render() %>
	<span id="environmentdiv"><%= form.getElement("environment").getDisplayName() %></span>
	...
	<tr>
		...
		<span id="labeldiv"><%= form.getElement("label").getDisplayName() %></span></td>
		<td>&nbsp</td>
		<td><b><font color=blue>*</font> Config Code</b></td>
	...
	<tr>
		<td nowrap><b><font color=blue>*</font> SoundScan Grp</b></td>
		<td><%= form.getElement("soundscan") %></td>
		<td>&nbsp;</td>
		<td><b>Title ID</b></td>
		<td><%= form.getElement("titleId") %></td>
	<tr>
	<tr bgcolor="white"><td colspan="6"></td></tr>
	...
	<tr>
		...
		<td>&nbsp</td>
		<td><b><font color=red>*</font> Format</b></td>
	...
	<tr>
		<td nowrap align="right">
			<b>Sell&nbsp;<%= form.getElement("test") %>
			<img src="<%= inf.getImageDirectory() %>filter_access1.gif" border=0>
		</td>
		<td><%= form.getElement("priceCode") %>&nbsp;&nbsp;&nbsp;<b>Retail</b>&nbsp;&nbsp;<%=retailCode%></td>
                <td>&nbsp;</td>
		<td colspan=1 nowrap>
			<b>Price</b>&nbsp;&nbsp;$ <%=price%>
                </td>
			...
			<%= form.getElement("specialPkgIndicator") %><b>Spec Pkg</b>
		</td>
	</tr>
	<tr>
		<td nowrap>
			<b>Dig. Price Code&nbsp;<%= form.getElement("testDPC") %>
			<img src="<%= inf.getImageDirectory() %>filter_access1.gif" border=0>
		</td>
		<td width="20"><%= form.getElement("priceCodeDPC") %></td>
    	<td>&nbsp</td>
		<td><b>Pkg&nbsp;&nbsp;&nbsp;</b></td>
		<td>&nbsp;<%= form.getElement("package") %></td>
	</tr>
	<tr>
    ...
    <%if(saveVisible)
    {
      String command = form.getStringValue("cmd");%>
		...
		<td align=center width="33%">
			<a id=glpAnchor href="JavaScript:submitGenerateLPN('LPNDown');"
		 		onMouseOver="Javascript:mtbLPNDown.over();return true;"
		 		onMouseOut="Javascript:mtbLPNDown.out()"
 				onClick="Javascript:mtbLPNDown.click(); return true;">
 			<img name='glp' id='glp' SRC="<%= inf.getImageDirectory() %>glp_On.gif" height="14" border="0" hspace="0" vspace="0" align="absmiddle">
			</a>
			<script type="text/javascript" language="JavaScript" >
		  	var mtbLPNDown = new ToggleButton(document,'<%= inf.getImageDirectory() %>', 'glp', 'JavaScript:submitGenerateLPN("LPNDown")', 66,14);
			</script>
		</td>
		<td align=center width="33%">
			<a href="JavaScript:submitTempLPN('TempLPNDown');"
 			...
 			<img name='tp' SRC="<%= inf.getImageDirectory() %>tp_On.gif" height="14" border="0" hspace="0" vspace="0" align="absmiddle">
			</a>
			<script type="text/javascript" language="JavaScript" >
		  	var mtbTempLPNDown = new ToggleButton(document,'<%= inf.getImageDirectory() %>', 'tp', 'JavaScript:submitTempLPN("TempLPNDown")', 66,14);
			</script>
		<!-- JR - PNR AFE -->
...
<table width="100%" bgcolor="lavender" border=0>
  <tr bgcolor="white">
  	<td colspan="6"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="4">
     <b>Origin Date:&nbsp;&nbsp;&nbsp;</b>
      ...
      <td colspan="1">
  	<b>Last Street Date Update:&nbsp;&nbsp</b>
...
<table width="100%" bgcolor="lavender" border=0>
  ...
  <tr>
	<td>
		<font color=red>* </font> Denotes Required Fields for all Physical Products <BR>
...
if (cmdString.equalsIgnoreCase("selection-edit-new") || cmdString.equalsIgnoreCase("selection-edit-copy"))
  isNew = true;
else
  isNew = false;
...
<div id="PackagingLayer" style="position:absolute;visibility:hidden;width:255px;height:60px;z-index:3;left:280px;top:220px;">
...
<div id="TerritoryLayer" style="position:absolute;visibility:hidden;width:255px;height:60px;z-index:3;left:280px;top:220px;">
<table width="251" border="1" cellspacing="0" cellpadding="1">
<tr bgcolor="wheat"><td>
	<table width="247">
	<tr>
...
<script>
     ...
     previousReleaseType= document.forms[0].releaseType.value;//jo ITS 44669 
</script>

<script>
var swtOn = parent.top.menuFrame.sOnColor;
var swtOff = parent.top.menuFrame.sOffColor;
// MSC Sitch menu frame selected tab to Labels Selection
...
//////////////////////////////////////////////////////
// MSC 01/06/04 ITS 821 Disable GLP button if
// releasing family is Canada
// show/hide the generate local product number button
//////////////////////////////////////////////////////
if(document.all.releasingFamily && document.all.releasingFamily.selectedIndex != -1)
{
  ReleasingFamilyChange(document.all.releasingFamily);
}
...
else if (context.getDelivery("ProjectNumberMessage") != null)
{
    ...
    //render the digital date if previously written to page before error occured
    out.println("noDigitalReleaseChanged();"); //jo ITS 44669 
...
%>
    ...
    //render the digital date if previously written to page before error occured
    noDigitalReleaseChanged();//jo ITS 44669 
