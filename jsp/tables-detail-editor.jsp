...
//global variables needed in include tables-detail-editor-javascript.js
  var form;
  var sort = "<%= MilestoneConstants.CMD_TABLES_DETAIL_SORT%>";
  var editor = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_TABLES_DETAIL_EDITOR)%>";
  var notepad = "<%= MilestoneConstants.NOTEPAD_TABLE_DETAIL%>";
  var cmd = "<%= form.getRenderableValue("cmd")%>";
  var deleteCommand = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_TABLES_DETAIL_EDIT_DELETE) %>";
  var save = "<%= MilestoneConstants.CMD_TABLES_DETAIL_EDIT_SAVE%>";
  var saveNew = "<%= MilestoneConstants.CMD_TABLES_DETAIL_EDIT_SAVE_NEW%>";
  var editNew = "<%= MilestoneConstants.CMD_TABLES_DETAIL_EDIT_NEW%>";
  var search = "<%= MilestoneConstants.CMD_TABLES_DETAIL_SEARCH%>";
...
function submitResize()
{
  ...
  parent.top.bottomFrame.location = "home?cmd=notepad-toggle&notepadType=<%=MilestoneConstants.NOTEPAD_TABLE_DETAIL_VISIBLE%>&lastCommand=<%=form.getRenderableValue("cmd")%>";
...
// release upgrade 8.1
function SetDigEquiv()
{
	if(document.getElementById("isDigitalEquivalent"))
	{ 
		// if Digital product type checked
		if(document.forms[0].ProdType[1].checked)
			document.getElementById("isDigitalEquivalent").disabled = true;
		else
			document.getElementById("isDigitalEquivalent").disabled = false;
	}
}
...
<script type="text/javascript" language="JavaScript" src="<%= inf.getHtmlDirectory() %>table-detail-editor-javascript.js"></script>
...
<table width="780" cellpadding="0" border="0">
  ...
  <td width="85"></td>
	...
	<td width="85">
			 <%
	      String link = "";
			  if (form.getRenderableValue("cmd").equalsIgnoreCase("tables-detail-edit-new") || form.getRenderableValue("cmd").equalsIgnoreCase("tables-detail-edit-save-new"))
        {
	        link = "<a href='JavaScript:submitSave(true)'";
        }
        else
        {
	        link = "<a href='JavaScript:submitSave(false)'";
        }
        %>
...
<%  if ( !(form.getRenderableValue("id").equals("26") ||
 ...
 <tr><td colspan=6 height=10>&nbsp;</td></tr>
  ...
  %>
  	...
  	<% /*This part of the code is made an include to make it easy to manage this page.
       	 ...
       	 <%@ include file="include-tables-detail-security-notepad.shtml" %>
 ...
 <%}%>
        ...
        <tr>
				  ...
				  <td><%= form.getElement("SubConfiguration") %></td>
                          <td class=secure><b><%=form.getElement("subdetinactive")%></b></td>
                                ...
                                <tr>
                                <!-- JR ITS 277 -- JP 6/19/03 -->
                                <td><b>Format/Schedule Type</b></td>
	...
	<%
           Table currentTable = (Table)context.getSessionValue("TableDetail");
         ...
         <tr>
            <td><b>Product Type</b></td>
               <%
               if (currentTable.getId() == 4 )
               {
                 if (!(form.getRenderableValue("cmd").equalsIgnoreCase("tables-detail-edit-new")))
                 {%>
                   <td><%= form.getStringValue("prodTypeLabel") %><%= form.getElement("prodType") %></td>
               <%}
               }
               else
               {%>
                 <td><%= form.getElement("prodType") %></td>
            <% } %>
         </tr>
         <%}%>
         ...
         {%>
         <tr>
            <td><b>Digital Equivalent</b></td>
            <td><%= form.getElement("isDigitalEquivalent") %></td>
         </tr>
         <%}%>
        ...
        <%}%>
				<tr><td colspan=2><b>Last Updated:</b>&nbsp;<%=form.getStringValue("lastUpdatedDate")%>&nbsp;<%=form.getStringValue("lastUpdatedBy")%></td></tr>
			  </table>
		    </td>
	      </tr>
          ...
          <table width="100%">
	        <tr>
              <th colspan="2">Table Header Search</th>
   	        ...
   	        <tr>
  		      <td class="label" nowrap>Group Description</td>
   		      <td><%= form.getElement("DescriptionSearch") %></td>
  	       </tr>
  	       <tr>
  		      <td class="label" nowrap>Digital Equivalent</td>
   		      <td><%= form.getElement("isDigitalEquivalentSearch") %></td>
  	       </tr>
  	       <tr>
