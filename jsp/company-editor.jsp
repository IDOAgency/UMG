...
<%
	...
	String htmlTitle = "Company Editor";
%>
...
//global variables needed in include company-editor-javascript.js
  var form;
  var sort = "<%= MilestoneConstants.CMD_COMPANY_SORT%>";
  var editor = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_COMPANY_EDITOR)%>";
  var notepad = "<%= MilestoneConstants.NOTEPAD_COMPANY%>";
  var cmd = "<%= form.getRenderableValue("cmd")%>";
  var deleteCommand = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_COMPANY_EDIT_DELETE) %>";
  var save = "<%= MilestoneConstants.CMD_COMPANY_EDIT_SAVE%>";
  var saveNew = "<%= MilestoneConstants.CMD_COMPANY_EDIT_SAVE_NEW%>";
  var editNew = "<%= MilestoneConstants.CMD_COMPANY_EDIT_NEW%>";
  var search = "<%= MilestoneConstants.CMD_COMPANY_SEARCH%>";
...
function submitResize()
{
  ...
  parent.top.bottomFrame.location = "home?cmd=notepad-toggle&notepadType=<%=MilestoneConstants.NOTEPAD_COMPANY_VISIBLE%>&lastCommand=<%=form.getRenderableValue("cmd")%>";
...
function processLoad(pFocusField,pAltField)
{
  ...
  //If company-editor - New - then
  //set focus field to be 'Parent1Selection'
  <% if (!form.getRenderableValue("cmd").equals("company-editor")) {%>
...
<script type="text/javascript" language="JavaScript" src="<%= inf.getHtmlDirectory() %>company-editor-javascript.js"></script>
...
<tr valign="middle" align="left">
  ...
  <td width="85"><span class="title">Company</span></td>
...
<tr>
  ...
  %>
  	...
  	<% /*	This part of the code is made an include to make it easy to manage this page.
       	 ...
       	 <%@ include file="include-company-security-notepad.shtml" %>
        ...
        <td valign="top" width="100%">
		  <table width="100%">
		    <tr>
			  ...
			  <td><%= form.getRenderableValue("Family") %></td>
			</tr>
                     <% }
                     %>
...
-->
			<tr>
			  <td valign="top" class="label">Division(s)</td>
...
-->
 ...
 <table width="100%" border="1" cellspacing="0" cellpadding="1">
   <tr>
     <td bgcolor="wheat">
	   <table width="100%">
	     <tr bgcolor="#BBBBBB"><th colspan="2">Company Search</th></tr>
	     <tr>
	       <td class="label">Family</td>
...
<%
 ...
 %>
   ...
   if(document.all.Family)
     document.all.Family.disabled = true;
