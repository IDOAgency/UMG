...
<%
	...
	String htmlTitle = "Family Editor";
...
//global variables needed in include family-editor-javascript.js
  var editor = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_FAMILY_EDITOR)%>";
  var notepad = "<%= MilestoneConstants.NOTEPAD_FAMILY%>";
  var cmd = "<%= form.getRenderableValue("cmd")%>";
  var deleteCommand = "<%= inf.getServletCmdURL(MilestoneConstants.CMD_FAMILY_EDIT_DELETE) %>";
  var save = "<%= MilestoneConstants.CMD_FAMILY_EDIT_SAVE%>";
  var saveNew = "<%= MilestoneConstants.CMD_FAMILY_EDIT_SAVE_NEW%>";
  var editNew = "<%= MilestoneConstants.CMD_FAMILY_EDIT_NEW%>";
  var search = "<%= MilestoneConstants.CMD_FAMILY_SEARCH%>";
...
function submitResize()
{
  ...
  parent.top.bottomFrame.location = "home?cmd=notepad-toggle&notepadType=<%=MilestoneConstants.NOTEPAD_FAMILY_VISIBLE%>&lastCommand=<%=form.getRenderableValue("cmd")%>";
...
<script type="text/javascript" language="JavaScript" src="<%= inf.getHtmlDirectory() %>family-editor-javascript.js"></script>
...
<% //start body %>
<body topmargin="0" leftmargin="0" onLoad="processLoad('CorporateDescription');">
...
<tr valign="middle" align="left">
	...
	<td width="85"><span class="title">Family</span></td>
...
<tr>
  ...
  %>
  	...
  	<% /*This part of the code is made an include to make it easy to manage this page.
       	 ...
       	 <%@ include file="include-family-security-notepad.shtml" %>
	...
	<td bgcolor="lavender" valign="top" colspan="6" width="100%">
		...
		<tr>
			<td valign="top" width="100%">
				<table width="100%">
					<tr><td class="label">Family</td>
					<td><%= form.getElement("CorporateDescription") %></td><td class="label">Active</td><td > <%= form.getElement("active") %></td>
        ...
        <% if (!(form.getRenderableValue("cmd")).equalsIgnoreCase(MilestoneConstants.CMD_FAMILY_EDIT_NEW))
           {%>
   				<tr>
  					<td valign="top" class="label">Environment(s)</td>
  ...
  <tr>
			<td valign="bottom">
			<% if (form.getRenderableValue("cmd").equals("family-editor"))
				{
		%>
	    <table>
        <tr>
...
<tr><td bgcolor="wheat">
	<table width="100%">
		<tr bgcolor="#BBBBBB"><th colspan="2">Family Search</th></tr>
		<tr>
