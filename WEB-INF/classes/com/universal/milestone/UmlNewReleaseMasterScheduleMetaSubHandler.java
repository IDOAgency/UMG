...
public class UmlNewReleaseMasterScheduleMetaSubHandler extends SecureHandler {
  ...
  protected static void fillUmlNewReleaseMasterScheduleForPrint(XStyleSheet report, Context context) {
    ...
    try {
      ...
      table_contents.setColWidth(0, 160);
      table_contents.setColWidth(1, 130);
      table_contents.setColWidth(2, 160);
      table_contents.setColWidth(3, 160);
      table_contents.setColWidth(4, 240);
      table_contents.setColWidth(5, 140);
      table_contents.setColWidth(6, 140);
      table_contents.setColWidth(7, 110);
      ...
      table_contents.setAlignment(0, 5, 36);
      ...
      table_contents.setObject(0, 0, "UPC");
      table_contents.setObject(0, 1, "P&D(*)\nLocal Prod #");
      table_contents.setObject(0, 2, "Rls Family - Label");
      table_contents.setSpan(0, 2, new Dimension(2, 1));
      table_contents.setObject(0, 4, "Artist");
      table_contents.setObject(0, 5, "Title  /");
      table_contents.setObject(0, 6, "Comments");
      table_contents.setObject(0, 7, "Plant");
      table_contents.setObject(0, 8, "Qty\nDone");
      table_contents.setObject(0, 9, "F/M");
      table_contents.setObject(0, 10, "BOM");
      table_contents.setObject(0, 11, "PRR");
      ...
      table_contents.setColBorderColor(0, 2, Color.white);
      table_contents.setColBorderColor(0, 1, new Color(208, 206, 206, 0));
      table_contents.setColBorderColor(0, 3, new Color(208, 206, 206, 0));
      table_contents.setColBorderColor(0, 4, new Color(208, 206, 206, 0));
      table_contents.setColBorderColor(0, 6, new Color(208, 206, 206, 0));
      table_contents.setColBorderColor(0, 5, Color.white);
      ...
      table_contents.setSpan(1, 0, new Dimension(NUM_COLS, 1));
          ...
          for (int x = 0; x < configsList.length; x++) {
            ...
            for (int i = 0; i < selections.size(); i++) {
              ...
              for (int plantCount = 0; plantCount < plantSize; plantCount++) {
                ...
                nextRow = 0;
                subTable.setColWidth(0, 160);
                subTable.setColWidth(1, 130);
                subTable.setColWidth(2, 160);
                subTable.setColWidth(3, 160);
                subTable.setColWidth(4, 240);
                subTable.setColWidth(5, 140);
                subTable.setColWidth(6, 140);
                subTable.setColWidth(7, 110);
                ...
                subTable.setAlignment(0, 2, 36);
                subTable.setAlignment(0, 3, 33);
                ...
                String mfgComment = (sel.getManufacturingComments() != null) ? sel.getManufacturingComments().trim() : "";
                ...
                if (p != null && p.getOrderQty() > 0)
                  poQty = (p != null && p.getOrderQty() > 0) ? Integer.toString(p.getOrderQty()) : "0"; 
                int poQtyNum = 0;
                try {
                ...
                if (p != null && p.getPlant() != null) {
                  ...
                  plantText = p.getPlant().getName();
                  String plantId = p.getPlant().getAbbreviation();
                  if (plantIdandName.get(plantId) != null) {
                    plant = (String)plantIdandName.get(plantId);
                  } else {
                    plant = "";
                  } 
                  int currentTotal = 0;
                  if (plantTotals.get(plantId) != null)
                ...
                nextRow = 0;
                if (i == 0 && plantCount == 0) {
                  nextRow = ReportHandler.insertLightGrayHeader(subTable, theStreetDateText, nextRow, 12);
                  ...
                  subTable.setSpan(nextRow, 0, new Dimension(NUM_COLS, 1));
                ...
                if (sel.getSpecialPackaging()) {
                  if (BOM.length() > 0) {
                    ...
                    subTable.setRowHeight(nextRow, 7);
                  } 
                ...
                subTable.setSpan(nextRow, 2, new Dimension(2, 2));
                subTable.setSpan(nextRow, 3, new Dimension(1, 2));
                subTable.setSpan(nextRow, 4, new Dimension(1, 2));
                subTable.setSpan(nextRow, 5, new Dimension(2, 2));
                ...
                if (idLength == 13 && pd.trim().equals("*"))
                  pd = "*"; 
                subTable.setObject(nextRow, 0, upc);
                subTable.setObject(nextRow, 1, String.valueOf(pd) + selectionNo);
                subTable.setObject(nextRow, 2, String.valueOf(releasingFamily) + " - " + label);
                subTable.setObject(nextRow, 4, artist);
                subTable.setSpan(nextRow, 5, new Dimension(2, 1));
                subTable.setSpan(nextRow + 1, 5, new Dimension(2, 1));
                subTable.setObject(nextRow, 5, titleComments);
                subTable.setObject(nextRow, 7, plant);
                subTable.setObject(nextRow, 8, MilestoneHelper.formatQuantityWithCommas(compQty));
                subTable.setObject(nextRow, 9, FM);
                subTable.setObject(nextRow, 10, BOM);
                if (tasks != null)
                  if (!hasPPRtask) {
                    subTable.setObject(nextRow, 11, "N/A");
                  } else if (MCvend != null && !MCvend.equals("")) {
                    subTable.setObject(nextRow, 11, String.valueOf(MC) + "\n" + MCvend);
                  } else {
                    subTable.setObject(nextRow, 11, MC);
                  }  
                subTable.setRowHeight(nextRow, 10);
                ...
                if (mfgComment != null && mfgComment.trim().length() > 0) {
                  subTable.setRowAutoSize(true);
                  subTable.setObject(nextRow, 5, mfgComment);
                  subTable.setFont(nextRow, 5, new Font("Arial", 2, 6));
      ...
      System.out.println(">>>>>>>>ReportHandler.fillUmlNewReleaseMasterScheduleMetaSubHandler(): exception: " + e + " \n ******\n " + e.getMessage());
    } 
  }
...
/* Location:              D:\Documents\NetBeansProjects\milestone2 Local.war!\WEB-INF\classes\co\\universal\milestone\UmlNewReleaseMasterScheduleMetaSubHandler.class
