    ...
    if (notepad != null) {
      ...
      if (MilestoneHelper.isStringNotEmpty(templateNameSrch))
        query = String.valueOf(query) + 
          MilestoneHelper.addQueryParams(query, " a.[name] " + MilestoneHelper.setWildCardsEscapeSingleQuotes(templateNameSrch)); 
      if (MilestoneHelper.isStringNotEmpty(configurationSrch) && !configurationSrch.equals("0"))
        query = String.valueOf(query) + 
          MilestoneHelper.addQueryParams(query, " a.[configuration] = '" + MilestoneHelper.escapeSingleQuotes(configurationSrch) + "'"); 
      if (MilestoneHelper.isStringNotEmpty(ownerSrch) && !ownerSrch.equals("0"))
        query = String.valueOf(query) + 
          MilestoneHelper.addQueryParams(query, " a.[owner] = '" + MilestoneHelper.escapeSingleQuotes(ownerSrch) + "'"); 
      if (MilestoneHelper.isStringNotEmpty(productSearch) && !productSearch.equals("2"))
        query = String.valueOf(query) + 
          MilestoneHelper.addQueryParams(query, " a.[productType] = " + MilestoneHelper.escapeSingleQuotes(productSearch)); 
