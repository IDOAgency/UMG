...
public class CapitolProductionSchedulePrintSubHandler extends SecureHandler {
  ...
  protected static void fillCapitolProductionUpdateForPrint(XStyleSheet report, Context context) {
    ...
    try {
      ...
      table_contents = new DefaultTableLens(1, 13);
      int nextRow = 0;
      ...
      table_contents.setColWidth(3, 120);
      ...
      columnHeaderTable.setColWidth(11, 250);
      System.out.println("--- Defining columns headings --- ");
      ...
      columnHeaderTable.setObject(nextRow, 6, "Copy to Art");
      columnHeaderTable.setObject(nextRow, 7, "Audio to\nManuf.");
      columnHeaderTable.setObject(nextRow, 8, "Art to\nPrinter");
      columnHeaderTable.setObject(nextRow, 9, "IO");
      columnHeaderTable.setObject(nextRow, 10, "Production Status");
      columnHeaderTable.setObject(nextRow, 11, "Comments/Pkg Info");
      ...
      for (int j = 0; j < selections.size(); j++) {
        ...
        String schedulingFormDate = "", labelCopyDate = "", copyToArtDate = "", audioToUMSDate = "", artToPrinterDate = "", productionStatusDate = "";
        String schedulingForm = "", labelCopy = "", copyToArt = "", audioToUMS = "", artToPrinter = "", productionStatus = "";
        Vector tasks = null;
        if (schedule != null)
        ...
        nextRow = 0;
        subTable = new DefaultTableLens(2, 13);
        ...
        subTable.setObject(nextRow, 6, copyToArtDate);
        subTable.setObject(nextRow, 7, audioToUMSDate);
        subTable.setObject(nextRow, 8, artToPrinterDate);
        subTable.setObject(nextRow, 9, "");
        subTable.setObject(nextRow, 10, productionStatusDate);
        subTable.setObject(nextRow, 11, sel.getSelectionComments());
        ...
        subTable.setObject(nextRow, 6, copyToArt);
        subTable.setObject(nextRow, 7, audioToUMS);
        subTable.setObject(nextRow, 8, artToPrinter);
        subTable.setObject(nextRow, 9, (poQty == 0) ? "" : Integer.valueOf(poQty));
        subTable.setObject(nextRow, 10, productionStatus);
        subTable.setObject(nextRow, 11, sel.getSelectionComments());
      ...
      System.out.println(">>>>>>>>ReportHandler.fillCapitolProductionUpdateForPrint(): exception: " + e);
      e.printStackTrace();
    } 
