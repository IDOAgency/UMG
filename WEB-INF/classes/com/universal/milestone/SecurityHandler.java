...
public class SecurityHandler extends SecureHandler {
  ...
  private boolean userSecuritySave(Context context) {
    ...
    if (UserManager.getInstance().isTimestampValid(user)) {
      ...
      if (environments != null) {
        this.log.log("envoroments no null");
        this.log.log("--" + environments.size());
        for (int i = 0; i < environments.size(); i++) {
          Environment environment = (Environment)environments.get(i);
          this.log.log("--" + environment.getName());
          if (environment != null && form.getElement("ue" + environment.getStructureID()) != null) {
            this.log.log("check box ue" + environment.getStructureID());
            if (((FormCheckBox)form.getElement("ue" + environment.getStructureID())).isChecked()) {
              this.log.log("check box ue" + environment.getStructureID() + "  checked");
              checkedEnvironments.add(environment);
            } 
          } 
        } 
      } 
      this.log.log("--" + checkedEnvironments.size());
      ...
      if (!UserManager.getInstance().isDuplicate(user)) {
        if (!form.isUnchanged()) {
          ...
          if (formValidation.isGood()) {
            ...
            this.log.log("checked companies");
            for (int j = 0; j < checkedEnvironments.size(); j++) {
              ...
              this.log.log("envoronments comanies for " + environmentCompanies.size());
              for (int k = 0; k < environmentCompanies.size(); k++) {
                Company envCompany = (Company)environmentCompanies.elementAt(k);
                this.log.log("envoronments comanies for " + envCompany.getName());
                if (!checkedCompanies.contains(envCompany))
                  checkedCompanies.add(envCompany); 
              } 
            } 
            this.log.log("enviroments conmpanies number" + checkedCompanies.size());
            if (user == null)
              this.log.log("User null"); 
            if (sessionUser == null)
              this.log.log("sessionUser null"); 
            if (checkedCompanies == null)
              this.log.log("checked companies is null"); 
  ...
  public Notepad getUsersNotepad(Context context) {
    ...
    Notepad notepad = new Notepad(contents, 0, 15, "Users", 7, columnNames);
    UserManager.getInstance().setUserNotepadQuery(context, notepad);
    return notepad;
  }
  ...
  public Notepad getUserCompanyNotepad(Vector contents, Context context, int userId) {
    ...
    return new Notepad(contents, 0, 15, "User-Environments", 21, columnNames);
  }
...
/* Location:              D:\Documents\NetBeansProjects\milestone2 Local.war!\WEB-INF\classes\co\\universal\milestone\SecurityHandler.class
