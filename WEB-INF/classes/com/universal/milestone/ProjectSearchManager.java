...
public class ProjectSearchManager implements MilestoneConstants {
  ...
  public static boolean sendEmailDistribution(Context context) {
    ...
    try {
      ...
      if (fileProps != null) {
        recipients = fileProps.getProperty("ProjectSearchEmailDistributionRecipients");
        copyRecipients = fileProps.getProperty("ProjectSearchEmailDistributionCopyRecipients");
        if (recipients == null) {
          log.debug("ProjectSearchManager: sendEmailDistribution:  no property 'ProjectSearchEmailDistributionRecipients' in " + MilestoneHelper_2.MILESTONE_CONFIG_FILE);
          recipients = "";
        } 
        if (copyRecipients == null) {
          log.debug("ProjectSearchManager: sendEmailDistribution: no property 'ProjectSearchEmailDistributionCopyRecipients' in " + MilestoneHelper_2.MILESTONE_CONFIG_FILE);
          copyRecipients = "";
        } 
      } 
      if (fileProps == null || (recipients.isEmpty() && copyRecipients.isEmpty())) {
        log.debug("ProjectSearchManager: sendEmailDistribution:  no emails sent out because of failing to get properties 'ProjectSearchEmailDistributionRecipients' and  'ProjectSearchEmailDistributionRecipients' in " + 
            MilestoneHelper_2.MILESTONE_CONFIG_FILE);
        return result;
      } 
...
/* Location:              D:\Documents\NetBeansProjects\milestone2 Local.war!\WEB-INF\classes\co\\universal\milestone\ProjectSearchManager.class
