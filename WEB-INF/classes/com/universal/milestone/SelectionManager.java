...
public class SelectionManager implements MilestoneConstants {
  ...
  public Selection getSelectionHeader(int id) {
    ...
    if (connector.more()) {
      ...
      String productionGroupCode = "";
      if (connector.getFieldByName("Production_Group_Code") != null)
        productionGroupCode = connector.getFieldByName("Production_Group_Code"); 
      selection.setProductionGroupCode(productionGroupCode);
  ...
  public Selection saveSelection(Selection selection, User updatingUser) {
    ...
    if (selection.getPriority())
      priority = 1; 
    System.out.println(">>>>>>>>>>>>>>>>>>>>>>> CompanyId: " + selection.getCompany().getStructureID());
    System.out.println(">>>>>>>>>>>>>>>>>>>>>>> Company: " + selection.getCompany().getName());
    System.out.println(">>>>>>>>>>>>>>>>>>>>>>> enviroment: " + selection.getEnvironment().getStructureID());
    System.out.println(">>>>>>>>>>>>>>>>>>>>>>> enviroment: " + selection.getEnvironment().getName());
    if (selection.getEnvironment() != null)
      environmentId = String.valueOf(selection.getEnvironment().getStructureID()); 
...
/* Location:              D:\Documents\NetBeansProjects\milestone2 Local.war!\WEB-INF\classes\co\\universal\milestone\SelectionManager.class
