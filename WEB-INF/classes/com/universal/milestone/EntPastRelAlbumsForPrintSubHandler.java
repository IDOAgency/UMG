...
public class EntPastRelAlbumsForPrintSubHandler extends SecureHandler {
  ...
  public static Vector getValidSelections(Vector allSelections) {
    ...
    for (int selectCounter = 0; selectCounter < allSelections.size(); selectCounter++) {
      ...
      if (fullLengthStrings.contains(subConfig)) {
        ...
        selectionHasAnAlbum(allSelections, projectID, releaseDateCalendar, fullLengthStrings, true, status)) {
        finalSelections.add((Selection)allSelections.elementAt(selectCounter));
      } 
    } 
  ...
  public static Vector getTitlesVector(Vector selections, Vector allFamilySelections) {
    ...
    for (int i = 0; i < selections.size(); i++) {
      ...
      if (fullLengthStrings.contains(subConfig)) {
        ...
        !selectionHasAnAlbum2(selections, projectID, releaseDateCalendar, fullLengthStrings, false, allFamilySelections, status)) {
        titleVector.add(((Selection)selections.elementAt(i)).getTitle());
      } 
    } 
    return titleVector;
...
/* Location:              D:\Documents\NetBeansProjects\milestone2 Local.war!\WEB-INF\classes\co\\universal\milestone\EntPastRelAlbumsForPrintSubHandler.class
