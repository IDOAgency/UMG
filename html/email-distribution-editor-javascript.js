 ...
 * Milestone v2
 *
 * File: email-distribution-editor-javascript.js
 */

function toggleSearch()
{
	toggle( 'searchLayer', 'firstNameSrch' );
...
function checkPage()
{
	if( !checkMandatoryField( document.all.firstName, "firstName" ) )
	{
		return false;
  }
	if( !checkMandatoryField( document.all.lastName, "LastName" ) )
	{
		return false;
  }
	return true;
...
function buildEmail()
{
  if(document.all.email.value == "") // if blank
  {
    document.all.email.value = document.all.firstName.value + "." +
                               document.all.lastName.value + "@umusic.com"
  }
}

function isValidEmailAddr(emailObj)
{
  var emailAddr = emailObj.value;
  // check for @ sign
  var pos = emailAddr.indexOf("@");
  if(pos  < 0)  // missing @
  {
    alert("Invalid Email Address entered....must contain an @");
    emailObj.focus();
    return false;
  }
  pos = emailAddr.indexOf(".",pos);
  if(pos  < 0)  // missing .
  {
    alert("Invalid Email Address entered....must contain an .");
    emailObj.focus();
...
{
  parent.top.bottomFrame.location = "home?cmd=email-distribution-editor";
...
{
  parent.top.bottomFrame.location = editor + "&distribution-id=" + pUserId;
