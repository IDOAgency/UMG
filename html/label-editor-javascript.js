 ...
 * Milestone v2
 *
 * File: label-editor-javascript.js
 */
...
function checkPage()
{
  ...
  if( !checkMandatoryField( document.all.CorporateAbbreviation, "Abbreviation" ) )
  {
    return false;
        ...
        // msc 12/10/02 check all the time
	//if (document.forms[0].cmd.value == editNew)
	//{
		if( !checkMandatoryField( document.all.Parent1Selection, "Division" ) )
        	{
    	          return false;
          	}
		else
...
{
  parent.top.bottomFrame.location = editor + "&label-id=" + pId;
