...
- 2006-01-20 jo ITS 44669 Added releaseTypeChanged() and verifyDigitalReleaseValue() to
  control the noDigitalRelease checkbox and digital date field when the user changes
  the release type 
  
- 2006-01-20 jo ITS 44669 Added check to submitSave() to ensure that digital date exists if
  commercial product and the No Digital Release checkbox is unchecked
  
- 2006-01-24 jo ITS 44669 Added check to submitSave() to ensure that the No Digital Release checkbox is checked
  for Promotional product

- 2006-01-25 jo ITS 44669 Modified noDigitalReleaseChange() and releaseTypeChanged() to only reset the
  digital date if the digital date is not already an empty string
  
- 2006-01-26 jo ITS 44669 Modified releaseTypeChanged() to ensure the digital date is always populated when 
  the release type is changed to Commercial
...
function saveComment()
{
  ...
  // mc its 966 show please wait message
  showWaitMsg();
...
function validateRequiredFields(imagePrefix) {
	...
	var strImprint = trim(document.forms[0].imprint.value);
	...
	//	}
   	//	return false;
		...
		alert("Please select 'Product Category', 'Release Type', 'Format' and 'Sub-Format' before saving!");
		if (imagePrefix != ''){
			eval( 'mtb' + imagePrefix ).reset();
		}
   		return false;
		...
		if (imagePrefix != '') {
   		...
   		return false;
	} else if(document.forms[0].status.value == "ACTIVE" && trim(document.forms[0].streetDate.value) == "") {
...
*/
		...
		alert("'SoundScan Grp' must be a numeric field.");//ITS 995 - jo
		if (imagePrefix != '') {
			eval( 'mtb' + imagePrefix ).reset();
		}
...
function submitGenerateLPN(imagePrefix){
  ...
  if(!validate_UPC_SSG())
  {
	...
	// check for LPNG required fields as well ITS 995 - jo
	else if(trim(document.forms[0].soundscan.value) == "") {
		alert("'SoundScan Grp' is required to generate a Local Product Number.  Please enter and submit again.");
		if (imagePrefix != '') {
			eval( 'mtb' + imagePrefix ).reset();
		}
		document.forms[0].configcode.focus();
	...
	// if it's a single however, skip this
        ...
        // JR - ITS 693 - modified the temp prefix, used to be MLST its now TEMP
        else if (!theCC.indexOf("S") == 0 &&
			(trim(document.forms[0].titleId.value)=="" ||
	         document.forms[0].titleId.value.indexOf("TEMP") > -1)) {
		...
		if (response){
			if( (trim(document.forms[0].prefix.value) == "" || document.forms[0].prefix.value == "-1") &&
				...
				(document.forms[0].selectionNo.value.indexOf("TEMP") > -1 || document.forms[0].selectionNo.value.indexOf("temp") > -1)) {
...
{
  // check to see if currently selected configuration code is digital equivalent
  var isDigitalEquivalent = PFMconfigs[document.getElementById("configcode").value];
	  		
  // jo ITS 44669
  // add check to ensure that the Digital Date field is populated
  // for Commercial products with the NoDigitalRelease checkbox set to false
  if (document.forms[0].releaseType.value == "CO" && !document.all.noDigitalRelease.checked){
       	// MC GDRS 11/06/2008
       	if(!IsGDRSactive) 
       	{
			document.all.digitalReleaseDate.innerHTML = document.all.streetDate.value;
       		document.all.digitalDate.value = document.all.streetDate.value;
       	}
  }
...
*/
		...
		alert("'SoundScan Grp' must be a numeric field.");
		if (imagePrefix != '') {
			eval( 'mtb' + imagePrefix ).reset();
		}
		document.forms[0].soundscan.focus();
	...
	*/
	...
	{
		// MC ITS 77987 GDRS message conditions, must be a GDRS related product
  		if(document.getElementById("GDRSProductStatus").value != "" && document.getElementById("GDRSProductStatus").value != "DELETE" )
	  	{
	  	  		
	  		//alert(isDigitalEquivalent);
	  		
	  		// message condition 1, if no digital release checkbox changed from true to false 
	  		// of the config code has been changed to a non digital equivalent
	  		if( (document.getElementById("noDigitalRelease").oldValue == "false" && document.getElementById("noDigitalRelease").checked) )
	  		{
	  			if(!confirm("Saving this release will delete the corresponding digital release in GDRS. Continue?")) 
	  			{
	  		    	// reset button
					if (imagePrefix != '')
					{
	  		...
	  		// message condition 2, street date or status value changed
	  		else if((document.getElementById("streetDate").oldValue != document.getElementById("streetDate").value) ||
	  		        (document.getElementById("status").oldValue != document.getElementById("status").value))
  			{  
  				if(!confirm("Changes to street/ship date and status will always update the associated GDRS digital release. Continue?"))
	  			{
	  		    	// reset button
					if (imagePrefix != '')
					{
...
function populateSell(obj, index, msg, isDigital)
{
	var indx = index.toUpperCase();
	var k = 0;
		
	if(isDigital)  // digital price codes
	{
	...
	else  // physical price codes
	{
	 	for( var j=0; j < p.length; j++)
		{
	   		 if( p[j][1].toUpperCase().substr(0,indx.length) == indx || !(index.length > 0) )
			{
				obj.options[k] = new Option(p[j][1], p[j][1]);
	      		k++;
	  		}
		}
	}
...
function clickSell(obj, isDigital)
{
  clearCombo(document.forms[0].priceCode);
  var msg = "";
  
  // if physical else digital
  if(!isDigital)  
  	populateSell(document.forms[0].priceCode, obj.value, "Not a valid sell code value ", false);
  else 
    populateSell(document.forms[0].priceCode, obj.value, "Not a valid Digital Price Code value ", true);
...
function clickSellDPC(obj)
{
  clearCombo(document.forms[0].priceCodeDPC);
  populateSell(document.forms[0].priceCodeDPC, obj.value, "Not a valid Digital Price Code value ", true);
  document.forms[0].priceCodeDPC.focus();
  return true;
} //end function clickSellDPC()
...
function validate_UPC_SSG()
{
  ...
  // msc 09/25/03 ITS 108 validate UPC & Sound scan group codes
  // physical product
  if(document.all.UPC != null && !checkLengthUPC( document.all.UPC.value, "UPC", 12, 14, "UPC", false, false ))
  {
     document.all.UPC.focus();
     return false;
  }
  if(document.all.soundscan != null && !checkLengthUPC( document.all.soundscan.value, "SoundScan Group code", 12, 14, "SSG", false, false ))
  {
  ...
  if(document.all.sound_scan_code != null && !checkLengthUPC( document.all.sound_scan_code.value, "Sales Grouping Code", 12, 14, "SSG", true, false ))
  {
...
//jo 12/22/2005 ITS 44669
//when the street/ship date's onBlur event is triggered, this function is called
//if noDigitalRelease is false and a Commercial product
//then set the div value in digitalReleaseDate
//and set the hidden digitalDate field-
function populateNoDigitalRelease()
{
   if (!document.all.noDigitalRelease.checked && (document.forms[0].releaseType.value == "CO") ){
       // MC GDRS 11/06/2008
       if(!IsGDRSactive)
       { 
       		document.all.digitalReleaseDate.innerHTML = document.all.streetDate.value;
...
//jo 12/22/2005 ITS 44669
//when the "no digital release" checkbox is clicked this function is called
//if the checkbox is unchecked and a Commercial product then the digital date field is updated
//otherwise, the digital date field is set to ""
function noDigitalReleaseChanged()
{
   if (!document.all.noDigitalRelease.checked && (document.forms[0].releaseType.value == "CO")){
	  // MC GDRS 11/06/2008
	  if(!IsGDRSactive)
      ...
      // MC GDRS 11/06/2008
      if(!IsGDRSactive)
      	document.all.digitalReleaseDate.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
      document.all.digitalDate.value = "";
   }
}
...
function releaseTypeChanged()
{
	...
	if (document.forms[0].releaseType.value == "CO") {
	
	   //if the previous release type was promo then
	   //announce that the No Digital Release Checkbox is about to be set to false
	   if (previousReleaseType!='0'){
	   	   // if force no digital release is not active
	       if(document.getElementById("ForceNoDigitalRelease").value != "true" && isDigitalEquivalent)
	       {
	       	 alert("Changing the release type from promo to commercial will uncheck the No Digital Release checkbox. If this product should not be available for digital release, please re-check this box.");
	       	 document.all.noDigitalRelease.checked = false;
	       }
	   }
	   if(!document.all.noDigitalRelease.checked){
	   		// mc GDRS 11/06/2008
	   		if(!IsGDRSactive)
	   		{
	       		document.all.digitalReleaseDate.innerHTML = document.all.streetDate.value;
	    ...
	    //moving to Promo so reset date and No Digital Release checkbox
        // MC GDRS 11/06/2008
        if(!IsGDRSactive) 
	        document.all.digitalReleaseDate.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
        ...
        document.all.noDigitalRelease.checked = true;
	   
	    // MC 01/07/08 clear the digital price code field
	    document.all.priceCodeDPC.selectedIndex = 0;
	    
	    for(var x=0; x < document.all.priceCodeDPC.options.length; x++)
	    	document.all.priceCodeDPC.options[x].selected = false;
	    	
	    document.all.priceCodeDPC.options[0].selected = true;
	} 
	
	previousReleaseType = document.forms[0].releaseType.value;
}

//jo 12/22/2005 ITS 44669
//This function is called when the noDigitalRelease checkbox's onBlur event is fired
function verifyDigitalReleaseValue()
{
	
}

// MC GDRS 11/06/2008 Set No Digital Release Flag 
function setNoDigitalRelease(obj)
{
	// if NDR flag changed manually, ignore config code setting
	//if(document.getElementById("noDigitalRelease").oldValue.toLowerCase() != document.getElementById("noDigitalRelease").checked.toString().toLowerCase())
	//	return;

	// set NDR value depending on config code is digital evuivalent status
	if(obj && obj.value != "")
	{ 
		// check to see if currently selected configuration code is digital equivalent
  		var isDigitalEquivalent = PFMconfigs[obj.value];
	
		// must be digital equivalent, not a releasing families that never triggers GDRS
		if(isDigitalEquivalent && document.getElementById("ForceNoDigitalRelease").value != "true")
			document.getElementById("noDigitalRelease").checked = false;
		else
			document.getElementById("noDigitalRelease").checked = true;
	}
}
