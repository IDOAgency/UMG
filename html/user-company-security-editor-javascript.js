 ...
 * Milestone v2
 *
 * File: user-company-security-editor-javascript.js
 */
...
{
	document.forms[0].cmd.value = "user-company-edit-new";
...
function submitDelete( imagePrefix )
{
  if(confirm("You are about to delete a record. Confirm?"))
	{
    document.forms[0].cmd.value = "user-security-company-delete";
    showWaitMsg();  // mc its 966 show please wait message
    document.forms[0].submit();
  }
  else
...
{
  parent.top.bottomFrame.location = editor + "&company-id=" + pCompanyId;
} //end function submitGet()
*/

function submitGet(pEnvironmentId)
{
  //alert("submitGet!" + editor + "&company-id=" + pCompanyId );
  //parent.top.bottomFrame.location = editor + "&company-id=" + pCompanyId;
  parent.top.bottomFrame.location = editor + "&environment-id=" + pEnvironmentId;
} //end function submitGet()
